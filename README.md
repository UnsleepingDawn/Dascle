...
> 不用注册账号，所有数据只存在你自己的手机上。

「Dascle」帮你把训练计划编排好、排到日程上，练的时候逐组记录，练完用图表回看进步。

唯一的联网行为是**检查新版本**：启动时向 GitHub 查一次最新 Release，有新版本才提示你去下载。除此之外不会发出任何请求，也不上传任何数据。

<!-- 截图：把 PNG 放进 docs/images/ 后，取消下面注释即可 -->

| 今日 | 训练日历 | 训练记录 | 统计 |
|---|---|---|---|
| ![今日](docs/images/today.jpg) | ![日历](docs/images/calendar.jpg) | ![训练](docs/images/workout.jpg) | ![统计](docs/images/stats.jpg) |
...
- 首次打开会自动导入 67 条内置动作（覆盖胸、背、腿、臀、肩、手臂、腹），开箱可用。

> 也可以装上 [Obtainium](https://github.com/ImranR98/Obtainium)，订阅本仓库后自动接收新版本。
> App 自己也会检查更新：启动时每天最多查一次，有新版本会弹窗提示（也可以在「我的 → 关于」里手动检查）。
...
| **统计** | 训练天数 / 训练次数 / 完成组数概览、肌群组数分布、动作重量进步 |
...
- **检查更新**：启动时每天最多向 GitHub 查一次最新版本，有新版本弹窗提示并跳到下载页；「我的 → 关于」里可以随时手动检查，设置里能关掉自动检查、也能给 GitHub 配一个镜像地址。
- **休息日也能开练**：今天被编排成休息日时，点一下休息图标可逐级「升级」到「开练！」，再选「使用明天的方案」（把之后最近的训练挪到今天、之后的排期整体提前）或「临时加一个方案」（直接开一场计划外训练）。

## 隐私

- 没有账号、没有云同步，训练数据不会离开你的手机。
- 唯一的联网行为是检查新版本：向 `api.github.com` 发一个只读请求获取最新 Release 信息（直连不通时可选用你配置的镜像前缀）。请求里不包含任何个人数据，App 也不会上传任何东西。
