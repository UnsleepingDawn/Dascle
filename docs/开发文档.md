...
核心能力：编排训练计划（routine）、把计划排到星期几、逐组记录训练（重量/次数或计时）、查看统计趋势、桌面「今日训练」组件。
...
app/src/main/
  java/com/fitplan/
    ...
    ui/{today,plan,stats,more}/ 各 Tab
    ui/workout/                 训练记录页
    ui/exercise/                动作选择器
...
- 计划页（新建/重命名/删除计划、动作选择器按肌群+器械筛选、动作编排与拖拽排序、目标值编辑）
- 日程页（按星期排期，每周循环）
...
- **「我的」Tab 目前是占位页**（`EmptyScreen` + `more_placeholder`）：动作库管理、体重记录、设置、关于页都还没做。
- `body_metric` 表与 `BodyMetricRepository` 已就绪，但没有 UI 调用。
- **提醒功能（AlarmManager / 通知渠道 / POST_NOTIFICATIONS）尚未实现**，manifest 里只有 `VIBRATE` 权限。
- `schedule_entry.specific_date`（一次性排期）schema 已支持，但 UI 只暴露了按星期排期。
