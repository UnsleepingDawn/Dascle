...
- 今日页（当天排期、未完成训练续练、开始训练入口、空态；**一天只有一个计划**——今天这次训练结束后计划卡片的「开始训练」置灰，下方出现随机鼓励语与「还想练？」卡片，点「再来！」把今天的训练重新置为进行中继续加练；今天这场训练没有对应计划卡片时（休息日「临时加一个方案」练出来的），鼓励语上面还会补一张训练总结卡，列方案名、动作数与每个动作的实际训练量；**今天在 `rest_day` 里时整页换成休息页**，只给一句随机休息寄语与「未完成的训练」，计划卡片与「还想练？」入口都不出现）
