...
- 计划 Tab = 训练日历（按月看每日肌群与实际训练、切月/回本月、点某天弹出明细；明细右下角「添加计划」把计划加到这一天——今天及以后写排期，已经过去的日子走 `RecordPastWorkout` 补记成一次已结束的训练（按计划目标铺满已完成组），「该天休息」把这一天标成休息日（顺带撤掉这一天的排期），两个动作互斥；实际训练条目与当天计划条目同尺寸，底色是肌群标签那套 `surfaceVariant` 灰，第二行写「完成 N 组」；点计划进编排页；休息日在格子里的日期底色与训练日同为蓝色系（今天及以后 `primary`，过去 `primaryContainer`），只有那行「休息」文字区分——今天及以后沿用 `tertiary` 深绿，过去改用肌群标签的 `surfaceVariant` 灰；休息日在明细里可取消）
