...
- 迁移文件目录：`data/src/main/sqldelight/com/fitplan/data/migrations/`（文件名是迁移**起始版本**，如 `4.sqm` 表示 schema 4→5；当前基线是 `1.db`，已经到 schema 5）。
...
- 计划 Tab = 训练日历（按月看每日肌群与实际训练、切月/回本月、点某天弹出明细；可对某天按「仅此日」或「每周」增删排期，点计划进编排页；编排出的休息日在格子里显示「休息」，明细里可取消）
