...
- **编排页**（`ui/plan/routine/`）：右下角两个入口——「+动作组」新建空组，「+」从动作库挑一个单独动作。
...
- 计划列表页（从日历顶栏右侧「设置单日方案」进入：新建/重命名/删除计划、动作选择器按肌群+器械筛选且会过滤计划里已有的动作、动作编排与拖拽排序、目标值编辑；右下角「+动作组」可建动作组，组卡片里设「做其中 x 个」并往里加动作，见 §7.11）
