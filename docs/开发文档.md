...
- 计划 Tab = 训练日历（按月看每日肌群与实际训练、切月/回本月、点某天弹出明细；明细右下角「添加计划」把计划排到这一天，「该天休息」把这一天标成休息日（顺带撤掉这一天的排期），两个动作互斥；点计划进编排页；休息日在格子里显示「休息」，明细里可取消。过去练过的日子显示实际肌群，其余情况（含事后补排的计划）显示排期肌群，过去的格子整体压淡）
