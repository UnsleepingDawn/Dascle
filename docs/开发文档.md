...
- 迁移文件目录：`data/src/main/sqldelight/com/fitplan/data/migrations/`（文件名是迁移**起始版本**，如 `5.sqm` 表示 schema 5→6；当前基线是 `1.db`，已经到 schema 13：`9.sqm` 给 `exercise` 加了 `metric` / `load_mode` / `default_reps`，`10.sqm` 加了 `routine_group` 与 `routine_exercise.group_id`，`11.sqm` 加了 `workout_exercise_state`，`12.sqm` 给 `routine_group` 加了 `name`）。
...
| `routine_group` | 动作组（组内放多个动作，「做其中 x 个」由 `max_picks` 决定，`name` 是可编辑的组名；见 §7.11） |
...
- 日历明细「今天及以后」的分支按**练没练二选一**（`PlanTab.DayDetailSheet`）：今天已经有实际训练
  （练完或中途退出都算，含 `GetMonthCalendar` 算进来的当天未结束训练）就只列「实际训练」，
  不再显示「当天计划」——计划已经落在这次训练上；今天还没练才列「当天计划」。
  所以今天一旦开练，明细里就没有移除排期的入口了，排期仍留在库里（与过去遗留排期一样），
  想改内容就编辑「实际训练」里的那次训练。
- 今日页的完成态：`GetTodayWorkoutSession` 取「`started_at` 落在今天」的那次训练（结束与否都算）。
  已结束（`WorkoutSession.isFinished`）时，计划卡片的「开始训练」置灰，列表里补一句随机鼓励语
  （`R.array.today_encouragements`，进页面时随机取一条）和一张同规格的「还想练？」卡片
  （`today_extra_title` / `today_start_extra`，点「再来！」把这次训练重新置为进行中继续加练）。
- 完成态的训练总结卡（`TodaySessionCard`）：今天这场训练**没有对应的计划卡片**时——休息日点小人
  「临时加一个方案」练出来的那种（`routineId == null`，今日页原本只剩一句鼓励语、看不出今天练了什么）——
  在鼓励语**上面**补一张总结卡片：方案名 + 动作数 + 每个动作的实际训练量。
  数据由 `TodayScreenModel.loadSessionExercises` 汇总：只统计已完成组（与日历「实际训练」、训练总结
  口径一致）；代表值取重量最大的那组（同重量取次数最多），计时动作取时间最长的一组，摘要稳定可预测；
  动作按各自最小 `setIndex` 排，跟训练记录里的顺序一致；动作库里已经找不到的动作跳过。
  界面（`TodaySessionCard.volumeText`）按计划卡片 `targetText()` 的写法拼成「3 组 × 10 次 · 60kg」，
  辅助类动作沿用 `weight_kg_assist`。有对应计划卡片（常规训练）时不给这张卡，免得与计划卡片重复；
  一个动作都没记录（组数全没勾）时也不给空卡片。休息日已完成分支与 `LazyColumn` 分支的位置一致，
  都插在鼓励语之前。
- 今天的休息日态优先级最高：`IsRestDay` 判定 `rest_day` 里有没有今天，命中则今日页整页换成休息页
  （`RestDayBlock`：休息图标 + 「今天休息」+ 随机一条 `R.array.today_rest_encouragements`）。
  该分支排在完成态之前；计划卡片与「还想练？」入口在休息日一律不给，只保留「未完成的训练」卡片
  （已开始的训练仍要有回去的路）。例外是今天已经练完的那场（事后把今天改成休息日）：照样给训练总结、
  鼓励语与「还想练？」卡片，否则今日页会退成一片「今天休息」。
...
- 「再来！」= `WorkoutLogScreen(sessionId, reopen = true)`：`WorkoutRepository.reopenSession`
  把这次训练 `finished_at` 置空重新变成进行中，之后加的计划外动作与组都追加到**同一次训练**上（与训练中左下角
  「添加计划外动作」等价），练完点「结束训练」写回新的 `finished_at`，并刷新桌面组件。
- 点「结束训练」写完 `finished_at` 后**直接退回今日页**（`WorkoutLogScreenModel.finishWorkout` 复用已有的
  `_exitTick` 让界面 `navigator.pop()`），不再停在只读汇总页；练了什么改由今日页计划卡片上的动作状态标注。
  `WorkoutLogScreen` 的只读汇总页（`WorkoutSummaryCard` + 只读动作卡 + 底部「完成」）保留给统计页历史列表
  点进来的详情用，`WorkoutSummary` / `workout_summary_done` 等都不再出现在结束流程里。
- 今日页计划卡片的动作状态（`TodayTab.TodayExerciseRow`）：今天这场训练就是照这张计划练的
  （`TodaySessionProgress.routineId` 与计划 id 对上）时，每个动作按今天练到哪了标注——
  **练过**（有已完成组）动作名前加一颗主色（蓝）圆圈勾（`Icons.Filled.CheckCircle` + `colorScheme.primary`，
  与动作选择页的选中标记同一套）；
  **跳过**与**动作组里没挑中**（`groupId != null` 且不在挑中集合里）只有动作名打删除线并转 `onSurfaceVariant` 灰，
  表示今天不用做，右侧的训练量只跟着变灰、不打删除线；其余保持原样（左侧给勾留同样宽度的空位，动作名才对得齐）。
  汇总在 `TodayScreenModel.loadSessionProgress`：已完成组取自 `workout_set`，
  跳过 / 挑中取自 `workout_exercise_state`，「组里挑中了谁」与记录页共用 domain 的 `pickedExerciseIds`。
  不区分结束态，训练中返回今日页也能看到进度；今天这场训练没有对应计划卡片时（休息日「临时加一个方案」练出来的），
  训练总结卡 `TodaySessionCard` 里的动作也按同样的带勾方式渲染。
...
- **存储**：`routine_group`（`position` + `max_picks` + `name`）与 `routine_exercise.group_id`（schema 10→11 的 `10.sqm`；`name` 是 schema 12→13 的 `12.sqm`）。
  `name` 可为空，为空表示没起过名字，界面回退到默认文案「动作组」。
...
- **编排页**（`ui/plan/routine/`）：底部并排两个蓝色按钮——「单独动作」从动作库挑一个排进来，「动作组」先弹窗二选一
  （「空动作组」= 新建一个空组，「预设动作组」= push `GroupPresetPickerScreen`）。
  组卡片标题显示 `routine_group.name`（改名的弹窗在卡片右侧的编辑图标里），可设「做其中 x 个」、编辑组内动作目标、往里加动作
  （动作选择器在 `groupId` 非空时切成多选 + 「完成」）。
  动作与动作组一起拖拽排序；组内顺序不提供拖拽，新加的动作追加到组末尾。
- **预设动作组**：预设数据在 `app/src/main/assets/group_presets.json`（`GroupPresetCatalog` 读取），
  每条含组名、所属部位、`maxPicks` 与组成动作在动作库里的名字。
  `GroupPresetPickerScreen` 只按部位筛选（没有器械那一排），单点一个预设组就整组加入计划并返回；
  计划里已经排过的组成动作会被跳过，`maxPicks` 跟着收敛到实际加入的动作数，加入后就是普通动作组。
...
- 训练记录页（逐组录入，按动作类型记重量/次数或重量/时长，纯自重动作不给重量框、也不能切换计量方式；自动组间休息倒计时 + 震动、加/减组、跳过动作、计划外动作、备注、结束汇总、暂时退出保留进度、放弃删除——减组后的组行数、跳过状态与组内挑选都即时写进 `workout_exercise_state`，暂时退出甚至杀进程重进都保持不变；动作组渲染成组卡，从中挑最多「做其中 x 个」个动作展开成子卡，记录能力与单独动作一致，见 §7.11；结束后的编辑态底部同样是「添加计划外动作」+ 红色「完成编辑」并排；点「结束训练」后直接退回今日页，练过 / 跳过 / 组内没挑中的动作在今日页计划卡片上分别以蓝圈勾、删除线 + 灰标出；练满目标组后按动作类型给出增重 / 增次数 / 增时间 / 减辅助重量的渐进提示，见 §7.10）
